...
deploy:
	@if [ -z "$$TRAVIS_PULL_REQUEST" ] || [ "$$TRAVIS_PULL_REQUEST" = "false" ]; then \
		if [ "$$TRAVIS_BRANCH" = "docker-cd" ]; then \
			if [ "$$SUITE" = "2017" ]; then \
				make deploy-2017; \
			fi; \
			if [ "$$SUITE" = "2018" ]; then \
...
deploy-2017: setup-aws access-ecr
	...
	./bin/ecs-deploy.sh --cluster $$ECS_CLUSTER --service-name $$ECS_SERVICE_2017 --image $$REMOTE_IMAGE_URL/civic-2017:latest
...
deploy-2018: setup-aws access-ecr
	...
	./bin/ecs-deploy.sh --cluster $$ECS_CLUSTER --service-name $$ECS_SERVICE_2018 -image $$REMOTE_IMAGE_URL/civic-2018:latest
