...
const getCampsiteSweepsDataTimepoints = data => data.map(item => new Date(item.properties.reportdate));
...
const splitTimepointsToMonths = (timepoints) => {
  ...
  const months = (endDate.getMonth() - startDate.getMonth()) + 1 + ((endDate.getFullYear() - startDate.getFullYear()) * 12);
  const sections = [...Array(months + 1).keys()];
  return sections.map(item => new Date(startDate.getFullYear(), startDate.getMonth() + item));
};

const filterSweepsDataByDate = (data, start, end) => data.filter((item) => {
  const d = new Date(item.properties.reportdate);
  return (+d >= +start && +d < +end);
});

const filterSweepsData = (data, timer) => {
  const timepoints = splitTimepointsToMonths(getCampsiteSweepsDataTimepoints(data));
