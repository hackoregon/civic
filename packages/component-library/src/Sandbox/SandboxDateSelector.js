...
const createMonths = (minDate, maxDate) => {
  ...
  const monthOptions = years.map((year, index, arr) => {
    let monthYears;
    if (index === 0) {
...
const dateHelper = (meta = {}) => {
  ...
  if (meta.dates && meta.dates.date_granularity !== null) {
    if (meta.dates.date_granularity.toLowerCase() === 'year') {
      dates = rangeRight(+meta.dates.min_date, +meta.dates.max_date + 1).map(String);
    }
    if (meta.dates.date_granularity.toLowerCase() === 'decade') {
      dates = rangeRight(+meta.dates.min_date, +meta.dates.max_date + 1, 10).map(String);
    }
